...
const helpers = require('./helpers-7c5be08e.js');
const index$1 = require('./index-98f0d0ce.js');
const cubicBezier = require('./cubic-bezier-78eeaf28.js');
const theme = require('./theme-91d9538d.js');
const frameworkDelegate = require('./framework-delegate-b4a07175.js');
    ...
    componentDidLoad() {
        {
            rIC(() => {
                ...
                if (!ionicGlobal.config.getBoolean('_testing')) {
                    Promise.resolve().then(function () { return require('./tap-click-e60c7a42.js'); }).then(module => module.startTapClick(ionicGlobal.config));
                }
                if (ionicGlobal.config.getBoolean('statusTap', isHybrid)) {
                    Promise.resolve().then(function () { return require('./status-tap-d5e1d8a0.js'); }).then(module => module.startStatusTap());
                }
                if (ionicGlobal.config.getBoolean('inputShims', needInputShims())) {
                    Promise.resolve().then(function () { return require('./input-shims-10c90fde.js'); }).then(module => module.startInputShims(ionicGlobal.config));
                }
                if (ionicGlobal.config.getBoolean('hardwareBackButton', isHybrid)) {
                    Promise.resolve().then(function () { return require('./hardware-back-button-50ef676f.js'); }).then(module => module.startHardwareBackButton());
                }
                if (typeof window !== 'undefined') {
                    Promise.resolve().then(function () { return require('./keyboard-aecec2ed.js'); }).then(module => module.startKeyboardAssist(window));
                }
                Promise.resolve().then(function () { return require('./focus-visible-b42370cb.js'); }).then(module => module.startFocusVisible());
            });
        }
    }
    render() {
...
const RouterOutlet = class {
    ...
    async connectedCallback() {
        this.gesture = (await Promise.resolve().then(function () { return require('./swipe-back-afc00f34.js'); })).createSwipeBackGesture(this.el, () => !!this.swipeHandler && this.swipeHandler.canStart() && this.animationEnabled, () => this.swipeHandler && this.swipeHandler.onStart(), step => this.ani && this.ani.progressStep(step), (shouldComplete, step, dur) => {
            if (this.ani) {
